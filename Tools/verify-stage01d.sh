...
ACCEPTANCE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/xyz-stage01d.XXXXXX")"
trap 'rm -rf "$ACCEPTANCE_DIR"' EXIT

printf '%s\n' "== Stage 01D rig contract and source checks =="
...
rg -q 'SDL_SCANCODE_F6|SDL_SCANCODE_F7' "$ROOT_DIR/Engine/src/Input.cpp"

printf '%s\n' "== Swift launcher tests and package =="
...
for CONFIGURATION in Debug Release; do
    ...
    printf '%s\n' "== $CONFIGURATION configure/build/tests/self-test =="
