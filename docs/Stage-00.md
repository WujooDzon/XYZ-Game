...
Stage 00 establishes the macOS developer workflow. The launcher remains the central build/configure/run tool; the current C++ target is documented in [Stage 01D](Stage-01D.md).
...
It runs all Swift tests, builds the launcher in Debug and Release, configures/builds the current game target in both configurations, runs the Stage 01D asset self-test, verifies a non-zero build exit, and cleans generated Release output in a temporary build tree.
